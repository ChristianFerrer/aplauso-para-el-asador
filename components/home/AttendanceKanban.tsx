...
const COLOR_THEMES: Record<string, Omit<BaseColumn, 'id' | 'label' | 'icon' | 'isCustom'>> = {
  violet: { bg: 'bg-violet-50', headerActive: 'bg-violet-500 text-white shadow-sm', headerIdle: 'bg-violet-100 text-violet-700 hover:bg-violet-200', cardOwn: 'bg-violet-500 text-white shadow-sm ring-2 ring-violet-300', cardOther: 'bg-white border border-violet-100 text-violet-700 shadow-card', drop: 'ring-2 ring-violet-400 bg-violet-100' },
  sky: { bg: 'bg-sky-50', headerActive: 'bg-sky-500 text-white shadow-sm', headerIdle: 'bg-sky-100 text-sky-700 hover:bg-sky-200', cardOwn: 'bg-sky-500 text-white shadow-sm ring-2 ring-sky-300', cardOther: 'bg-white border border-sky-100 text-sky-700 shadow-card', drop: 'ring-2 ring-sky-400 bg-sky-100' },
  teal: { bg: 'bg-teal-50', headerActive: 'bg-teal-500 text-white shadow-sm', headerIdle: 'bg-teal-100 text-teal-700 hover:bg-teal-200', cardOwn: 'bg-teal-500 text-white shadow-sm ring-2 ring-teal-300', cardOther: 'bg-white border border-teal-100 text-teal-700 shadow-card', drop: 'ring-2 ring-teal-400 bg-teal-100' },
  pink: { bg: 'bg-pink-50', headerActive: 'bg-pink-500 text-white shadow-sm', headerIdle: 'bg-pink-100 text-pink-700 hover:bg-pink-200', cardOwn: 'bg-pink-500 text-white shadow-sm ring-2 ring-pink-300', cardOther: 'bg-white border border-pink-100 text-pink-700 shadow-card', drop: 'ring-2 ring-pink-400 bg-pink-100' },
  lime: { bg: 'bg-lime-50', headerActive: 'bg-lime-500 text-white shadow-sm', headerIdle: 'bg-lime-100 text-lime-700 hover:bg-lime-200', cardOwn: 'bg-lime-500 text-white shadow-sm ring-2 ring-lime-300', cardOther: 'bg-white border border-lime-100 text-lime-700 shadow-card', drop: 'ring-2 ring-lime-400 bg-lime-100' },
  indigo: { bg: 'bg-indigo-50', headerActive: 'bg-indigo-500 text-white shadow-sm', headerIdle: 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200', cardOwn: 'bg-indigo-500 text-white shadow-sm ring-2 ring-indigo-300', cardOther: 'bg-white border border-indigo-100 text-indigo-700 shadow-card', drop: 'ring-2 ring-indigo-400 bg-indigo-100' },
}
...
export default function AttendanceKanban({ guests: initialGuests, userId, eventId, isAdmin, customStatuses = [] }: Props) {
  ...
  const allColumns: BaseColumn[] = [
    ...
    ...customStatuses.map(cs => ({
      id: cs.id, label: cs.name, isCustom: true,
  ...
  function handleScroll() {
    ...
    setActiveCol(Math.round(fraction * (sortedColumns.length - 1)))
  }
        ...
        <form onSubmit={handleAddStatus} className="mb-3 p-3.5 bg-gradient-to-br from-orange-50 to-rose-50 rounded-2xl border border-orange-100">
          ...
          <div className="flex gap-1.5 mb-2">
            {COLOR_SWATCHES.map(s => (
              <button key={s.key} type="button" onClick={() => setNewStatusColor(s.key)}
          ...
          return (
            ...
            >
                  ...
                  confirmDeleteStatus === col.id ? (
                    <div className="flex gap-0.5 flex-shrink-0">
                      <button onClick={() => handleDeleteStatus(col.id)} disabled={isStatusPending}
                        className="w-5 h-5 rounded-lg bg-rose-500 text-white flex items-center justify-center">
                        <Check className="w-3 h-3" strokeWidth={2.5} />
                      </button>
                      <button onClick={() => setConfirmDeleteStatus(null)}
                        className="w-5 h-5 rounded-lg bg-stone-200 text-stone-600 flex items-center justify-center">
                        <X className="w-3 h-3" strokeWidth={2.5} />
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmDeleteStatus(col.id)}
                      className="flex-shrink-0 w-5 h-5 rounded-lg bg-white/50 text-stone-400 hover:text-rose-400 flex items-center justify-center transition-colors">
              ...
              {colGuests.map(guest => {
                ...
                const firstName = (guest.profile?.name || '?').split(' ')[0]
                const plusOnes = guest.plus_ones ?? 0
                return (
                  <div
                    key={guest.id}
                    draggable={canDrag}
                    onDragStart={canDrag ? e => onCardDragStart(e, guest.user_id) : undefined}
      ...
      <div className="flex justify-center gap-1.5 mt-3 sm:hidden">
        {sortedColumns.map((_, i) => (
          <div key={i} className={cn(
            ...
            i === activeCol ? 'w-4 h-1.5 bg-gradient-to-r from-orange-500 to-rose-500' : 'w-1.5 h-1.5 bg-stone-200'
          )} />
        ))}
      </div>
